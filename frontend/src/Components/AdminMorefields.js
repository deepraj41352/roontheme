...
export default function AdminMorefields() {
  ...
  const assignedAgentByCateHandle = (index) => {
    ...
    if (category) {
      const selectedCategory1 = fatchCategory.find(
        (categoryItem) => categoryItem._id === category
      );
      const agentsForCategory = fatchAgent.filter(
  ...
  const handleAgentChange = (index, key, value) => {
    ...
    const categoryName = fatchCategory.find(
          ...
          <Card className="chatboxes">
            ...
            <Card.Body>
              <Link to={`/chatWindowScreen/`}>
                <Button className="chatBtn" type="button"></Button>
              </Link>
            </Card.Body>
          </Card>
          );
